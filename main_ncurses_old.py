...
Based on the architecture from DnD-5e-ncurses project
"""
...
try:
    ...
    from populate_functions import (populate, request_monster, request_armor, request_weapon,
                                    request_equipment, request_equipment_category)
    ...
    try:
        from main import (create_new_character, generate_random_character, display_character_sheet,
                         menu_read_options, delete_character_prompt_ok, rename_character_prompt_ok,
                         explore_dungeon, generate_encounter_levels, load_encounter_table,
                         load_encounter_gold_table, load_xp_levels as load_xp_levels_func, generate_encounter)
    except ImportError as e:
        # These functions are optional - create stubs
        def create_new_character(roster):
            return Character()
        def generate_random_character(roster, races, subraces, classes, names, human_names, spells):
            return Character(name=f"Hero{len(roster)+1}")
        def display_character_sheet(char):
            pass
        def menu_read_options(char, roster):
            pass
        def delete_character_prompt_ok(char):
            return True
        def rename_character_prompt_ok(char, new_name):
            return True
        def explore_dungeon(party, monsters):
            pass
        def generate_encounter_levels(party_level):
            return [1, 2, 3, 4, 5]
        def load_encounter_table():
            return {}
        def load_encounter_gold_table():
            return [10, 20, 30, 50, 100]
        def load_xp_levels_func():
            return [0, 300, 900, 2700, 6500, 14000]
        def generate_encounter(available_crs, encounter_table, encounter_level, monsters, monster_groups_count, spell_casters_only):
            return []

    IMPORTS_AVAILABLE = True
    print("[IMPORTS] Successfully loaded game modules with populate_functions")

except ImportError as e:
    # Complete fallback if even populate_functions fails
    IMPORTS_AVAILABLE = False
    print(f"[IMPORT ERROR] Failed to import game modules: {e}")
    print("[IMPORT ERROR] Using stub functions - weapons/armors will not be available")

    class Character:
        def __init__(self, name="Hero", level=1):
            self.name = name
            self.level = level
            self.hit_points = 10
            ...
            self.id_party = -1
            self.class_type = type('ClassType', (), {'name': 'Fighter', 'can_cast': False})()
    ...
    def load_xp_levels_func():
        return [0, 300, 900, 2700, 6500, 14000, 23000, 34000, 48000, 64000,
...
class DnDCursesUI:
    ...
    def load_game_data(self):
        ...
        try:
            ...
            # Load databases
            result = load_dungeon_collections()
            if result:
                self.monsters, self.armors, self.weapons, self.equipments, \
                self.equipment_categories, self.potions = result
            ...
            if IMPORTS_AVAILABLE:
                try:
                    ...
                    self.push_message(f"Error loading encounter data: {str(e)[:30]}")
                    self.encounter_table = {}
                    self.encounter_gold_table = []
                    self.available_crs = []
            else:
            ...
            self.push_message(f"Error loading game data: {str(e)}")
    ...
    def draw_inn_rooms(self, lines: int, cols: int, character):
        ...
        try:
            ...
            rooms = [
                ...
                ("A Cot (10 GP / Week)", 10, 1),
                ("Economy Room (100 GP / Week)", 100, 3),
                ("Merchant Suites (200 GP / Week)", 200, 7),
                ("The Royal Suites (500 GP / Week)", 500, 10)
            ]
    ...
    def draw_temple_menu(self, lines: int, cols: int):
        ...
        try:
            ...
            candidates = [(c, cures_costs.get(c.status, 0) * c.level)
                         for c in self.roster if c.status in cures_costs]
    ...
    def draw_buy_items(self, lines: int, cols: int, character):
        ...
        try:
            ...
            # Get available items: weapons + prof_armors (like main.py)
            items = []
            debug_msgs = []
    ...
    def draw(self):
        ...
        if self.mode == 'main_menu':
            ...
            self.draw_char_select_menu(lines, cols,
                [c for c in self.roster if c not in self.party], "Select Character from Roster")
    ...
    def _handle_sell_items(self, c: int) -> None:
        ...
        if c in (curses.KEY_DOWN, ord('j')):
            ...
            if inventory_items and self.sell_cursor < len(inventory_items):
                ...
                # Check if equipped (like main.py line 1321-1327)
                from dao_classes import Weapon, Armor
                if isinstance(item, (Weapon, Armor)) and hasattr(item, 'equipped') and item.equipped:
                ...
                if len(self.roster) >= MAX_ROSTER:
                    ...
                    try:
                        if IMPORTS_AVAILABLE:
                            ...
                            new_char = generate_random_character(self.roster, self.races, self.subraces,
                                                                self.classes, self.names, self.human_names, self.spells)
                            self.roster.append(new_char)
                            save_character(new_char, _dir=self.characters_dir)
        ...
        if c in (curses.KEY_DOWN, ord('j')):
            ...
            if available_chars and self.roster_cursor < len(available_chars):
                ...
                if self.previous_mode == 'tavern':
                    ...
                    # Delete character
                    if IMPORTS_AVAILABLE and delete_character_prompt_ok(selected_char):
    ...
    def _use_item_from_inventory(self) -> None:
        ...
        if self.inventory_item_cursor < len(potions):
            ...
            try:
                ...
                self.character_viewing.hit_points = min(
                    self.character_viewing.hit_points + heal_amount,
                    self.character_viewing.max_hit_points
                )
        ...
        if not hasattr(self, 'dungeon_state'):
            ...
            if IMPORTS_AVAILABLE:
                try:
                    ...
                    self.dungeon_state['encounter_levels'] = [party_level] * 5
            else:
                self.dungeon_state['encounter_levels'] = [party_level] * 5
    ...
    def _character_attack(self, character):
        ...
        if IMPORTS_AVAILABLE and hasattr(character, 'attack'):
            try:
                ...
                try:
                    # Call the actual attack method (may use spells, weapons, etc.)
                ...
                if output:
                    # Parse output lines and add to dungeon log
        ...
        # Note: target.hit_points is already modified by character.attack()
        # Only check for death
        if target.hit_points <= 0:
    ...
    def _distribute_rewards(self):
        ...
        if IMPORTS_AVAILABLE and hasattr(self, 'encounter_gold_table') and self.encounter_gold_table:
            try:
