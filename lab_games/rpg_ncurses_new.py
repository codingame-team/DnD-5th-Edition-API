...
def combat(stdscr, player, enemy):
    stdscr.nodelay(False)  # Block input in combat
    ...
    stdscr.addstr(12, 1, "Combat terminé! Appuyez sur une touche.")
    stdscr.refresh()
    stdscr.getch()
    stdscr.nodelay(True)  # Resume non-blocking input

def handle_input(key, stdscr, player, game_map, enemies):
    if key == -1:
        return
    move_map = {...}  # same as before
    if key in move_map:
        dx, dy = move_map[key]
...
def main(stdscr):
    ...
    camera = Camera(len(game_map[0]), len(game_map), term_w, term_h - 2)
    ...
    while True:
        ...
        if key == curses.KEY_RESIZE:
            term_h, term_w = stdscr.getmaxyx()
            camera.view_width = min(term_w // SPACING, len(game_map[0]))
            camera.view_height = min(term_h - 2, len(game_map))
        handle_input(key, stdscr, player, game_map, enemies)
        camera.follow(player)
        stdscr.clear()
        draw_map(stdscr, game_map, player, enemies, camera)
        stdscr.addstr(camera.view_height, 0,
                      f"HP: {player.hp} | Pos: ({player.x},{player.y})")
        stdscr.refresh()
