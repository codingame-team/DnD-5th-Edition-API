...
# Compute normal vectors to cube faces
cube_normals = [
    (0, 0, 1),  # Front face
    (0, 0, -1),  # Back face
    (1, 0, 0),  # Right face
...
# Compute camera direction vector (assuming camera looks along -z axis)
...
while True:
    ...
    # Draw cube faces
    for i, normal in enumerate(cube_normals):
        # Compute dot product between face normal and camera direction
        dot_product = sum(a * b for a, b in zip(normal, camera_direction))

        # If dot product is positive, face is visible, otherwise, skip
        if dot_product >= 0:
            # Draw face
            face_points = [points[index] for index in cube_faces[i]]
            pygame.draw.polygon(window, (255, 255, 255), face_points)
