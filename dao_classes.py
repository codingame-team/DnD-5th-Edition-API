    ...
    def attack(self, character: Character) -> int:
        ...
        if castable_spells:
            ...
            if attack_spell.level > 0:
                self.spell_slots[attack_spell.level - 1] -= 1
            damage_dices: List[DamageDice] = attack_spell.get_spell_damages(caster_level=self.caster_level, ability_modifier=self.ability_modifier)
            for damage in damage_dices:
                if damage.dice is None:
                    total_damage += damage.bonus
                else:
                    dice_count, roll_dice = map(int, damage.dice.split('d'))
                    total_damage += sum([randint(1, roll_dice) for _ in range(dice_count)]) + damage.bonus
    ...
    def get_spell_damages(self, caster_level: int, ability_modifier: int) -> List[DamageDice]:
        ...
        if self.damage_at_slot_level:
            ...
            if str(caster_level) in self.damage_at_character_level:
                damage_dice = self.damage_at_character_level.get(str(caster_level))
            else:
        ...
        if damage_dice and '+' in damage_dice:
            if damage_dice.count('d') == 0:
                damage_dices.append(DamageDice(dice=None, bonus=20))
            elif damage_dice.count('d') > 1:
                for dd in damage_dice.split('+'):
                    damage_dices.append(DamageDice(dice=dd.strip(), bonus=0))
            else:
...
class Character:
    ...
    def attack(self, monster: Monster, order: int):
        ...
        if castable_spells:
            ...
            damage_dices: List[DamageDice] = attack_spell.get_spell_damages(caster_level=self.level, ability_modifier=ability_modifier)
            damage_roll: int = 0
            for damage in damage_dices:
                if damage.dice is None:
                    damage_roll += damage.bonux
                else:
                    dice_count, roll_dice = map(int, damage.dice.split('d'))
                    damage_roll += sum([randint(1, roll_dice) for _ in range(dice_count)]) + damage.bonus
            cprint(f'{color.GREEN}{self.name}{color.END} ** CAST SPELL {attack_spell.name.upper()} **')
            if attack_spell.dc_type is None:
                # No saving throw available for this spell!
